...
### Контрольные вопросы:
ACL vs Object Ownership (Enforced)  
ACL — старый способ: доступ к файлам задаётся вручную (owner, public-read и т.д.).  
Object Ownership (Enforced) — современный: все объекты принадлежат владельцу бакета, ACL отключены, доступ контролируется политиками (IAM, Bucket Policy).  
  
Block all public access  
Опция блокирует любые публичные права (через ACL или Policy). Нужна, чтобы никто случайно не сделал файлы доступными всем пользователям интернета.  
  
Object key vs file name  
Имя файла — это просто название файла локально (например user1.jpg).  
Object key — полный путь объекта в бакете (например avatars/user1.jpg).  
  
cp, mv, sync и --acl public-read  
  
cp — копирует файлы.  
  
mv — перемещает (копирует и удаляет оригинал).  
  
sync — синхронизирует каталоги (загружает только изменения).  
Флаг --acl public-read делает объект доступным всем (публичным).  

Отключение версионирования  
Старые версии сохранятся, но новые объекты больше не будут версионироваться (версий не создаётся).  

Storage Class  
Это “уровень хранения”: определяет цену и скорость доступа.  
Напр., Standard (частый доступ), Glacier (дешево, для архива), IA (редкий доступ).  

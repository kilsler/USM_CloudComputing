...
3. Выберите созданный ранее VPC и добавьте 2 приватные подсети из 2 разных AZ.

4. Создайте экземпляр базы данных Amazon RDS (Databases -> Create database).  
...
<img width="1635" height="325" alt="image" src="https://github.com/user-attachments/assets/e2b5f3fe-8b44-4a0c-8747-1028ef70c71f" />  

<img width="1912" height="920" alt="image" src="https://github.com/user-attachments/assets/f05e9cc1-081e-4447-bf85-fd2aa0548aac" />
...
1. Какие данные вы видите при подключении к ReadReplica ? Объясните почему. При подключении к Read Replica и выполнении запроса на чтение были отображены данные, совпадающие с данными основного экземпляра базы.
2. Получилось ли выполнить запись на Read Replica? Почему?При выполнении запроса на запись на реплике операция не была выполнена. Запрос завершился ошибкой о том, что сервер работает в режиме read-only.
3. Отобразилась ли новая запись на реплике? Объясните почему.После добавления новой записи на основном экземпляре и повторного выполнения SELECT на реплике новая запись стала отображаться. Это происходит благодаря механизму асинхронной репликации.
4. Объясните, зачем нужны Read Replicas и в каких сценариях их использование будет полезным. Read Replicas используются для разгрузки основного экземпляра базы данных за счёт переноса операций чтения на отдельные реплики.
5. Что такое Subnet Group? И зачем необходимо создавать Subnet Group для базы данных? Subnet Group — это логическое объединение нескольких частных подсетей (subnets) внутри VPC.
6. Какие преимущества и недостатки использования DynamoDB по сравнению с реляционной базой данных Amazon RDS в вашем случае?  
+В отличие от реляционной БД, масштабирование в DynamoDB происходит автоматически. Нет необходимости вручную увеличивать размер инстансов, что снижает нагрузку на администрирование.  
+DynamoDB обеспечивает очень быстрый доступ к данным благодаря архитектуре NoSQL и распределённому хранению.  
-DynamoDB не поддерживает SQL и не позволяет выполнять JOIN, субзапросы, сложную фильтрацию или агрегацию.  
-Хотя транзакции поддерживаются, их возможности значительно уже, чем в классических реляционных СУБД.  
